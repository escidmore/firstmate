...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; ship tasks may also carry issue_key=, delivery_title_rule=, and delivery_link_rule= from intake; an optional traceparent= only when trace context is enabled (docs/configuration.md "Trace context propagation"); kind=secondmate also records home= and projects=, plus remote_host=/remote_root=/remote_backend=/remote_herdr_session=/remote_target= for a remote route; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and a required valid provider-observed pr_head= (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); registration fails closed when the head is unavailable or invalid; fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for a Relay-originated task (section 14)
...
Run `bin/fm-pr-check.sh <id> <PR url>` - it validates any required issue title and link against task intake metadata before it records canonical `pr=` and a required valid provider-observed `pr_head=` or arms the watcher's merge poll; registration fails closed when the head is unavailable or invalid.
...
When a project requires an issue key, carry the intake key through the generated brief and spawn metadata rather than deriving it from the task id.
