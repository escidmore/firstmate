...
When intake supplies an issue key and title/link rules, the generated brief and spawn metadata carry them, and `fm-pr-check.sh` validates provider fields before recording `pr=` or arming merge monitoring; `fm-pr-merge.sh` uses the same validation before merging.
